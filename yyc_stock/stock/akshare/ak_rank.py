...
class AK_RANK(AkshareBase):
    ...
    def register_router(self):
        ...
        @self.router.get("/rank/up")
        async def rank_up(req:Request):
            """获取上升最快的股票排名前100"""
            try:
                #df = self._get_df_source(db_name="price_30.db",sql=f"select * from price where code='{code}'")
                #codes = self._get_request_codes(req)
                df = self._get_df_source(ak_func=ak.stock_hot_up_em,columns={})
                df = self._prepare_df(df,req)
                content = self._to_html(df,formats=req.query_params.get('f'),fix_columns=['代码','名称','换手率'])
                return HTMLResponse(content=content)
            except Exception as e:
        ...
        @self.router.get("/rank/realtime")
        async def rank_realtime(req:Request):
            """获取某只股票的实时排名"""
            try:
                #df = self._get_df_source(db_name="price_30.db",sql=f"select * from price where code='{code}'")
                codes = self._get_request_codes(req)
                code_info = self._get_stock_code(codes[0])
                code = code_info['mr_code']
                mc = code_info['name']
                print(code_info)
                df = self._get_df_source(ak_func=ak.stock_hot_rank_detail_realtime_em,symbol=code,columns={})
                df.insert(0,'mc',mc)
        ...
        @self.router.get("/rank/relate")
        async def rank_relate(req:Request):
            """获取某只股票关联的股票"""
            def _get_name(x):
                try:
                    return self._get_stock_code(x)['name']
                except:
                    return 'N/A'
            try:
                #df = self._get_df_source(db_name="price_30.db",sql=f"select * from price where code='{code}'")
                ...
                mc = code_info['name']
                print(code_info)
                df = self._get_df_source(ak_func=ak.stock_hot_rank_relate_em,symbol=code,columns={})
                df.insert(0,'mc',mc)
                df['相关股票名称'] = df['相关股票代码'].apply(lambda x:_get_name(x))
                df = self._prepare_df(df,req)
                content = self._to_html(df,formats=req.query_params.get('f'),fix_columns=['代码','名称','换手率'])
                return HTMLResponse(content=content)
            except Exception as e:
